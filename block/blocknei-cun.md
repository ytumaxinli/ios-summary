...
> ```
>
> > 不使用外部变量 或 使用全局变量、外部静态变量（全局/局部）时block的，无论使用什么修饰符block类型都为**\_\_NSGlobalBlock\_\_**，即全局区block。
>
> > 使用 非全局变量 或 外部静态变量（全局/局部）时
> >
> > 1. 不使用修饰符 和 使用strong、copy、weak三种修饰符的block均为\_\_NSMallocBlock\_\_，即堆区block；
